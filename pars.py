...
class getRasp:
    ...
    def newRasp(self,newGroup,newProf,newBlock,newAud):
        ...
        if "Расписание для группы " in rasp[0] or rasp[0]=='':
            ...
            self.week.append(rasp[weekid[5]+2:len(rasp) - 1])
            self.week.append("")
            self.weekstr = [[""]] * 7
            #print(self.week)
    ...
    def printRasp(self):
        print(self.legends,self.info,self.offschedule,sep='\n')
        for i in range(len(self.week)):
            print(self.week[i],end='\n')
