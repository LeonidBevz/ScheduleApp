...
class getRasp:
    ...
    def __init__(self):
        ...
        if not self.urlok:  # --------проверка есть ли доступ к сайту-----------#
    ...
    def newRasp(self, newGroup, newProf, newBlock, newAud):
        ...
        d = requests.get(
            "https://guap.ru/rasp/" + "?g=" + self.groupid[newGroup] + "&p=" + self.profid[newProf] + "&b=" +
            self.blockid[newBlock] + "&r=" + self.audid[newAud])
        ...
        if rasp[0] == '' or "Нет данных, удовлетворяющих условиям поиска ..." in rasp[1]:
            ...
            if 'Вне сетки расписания' in self.info:
                ...
                for i in range(len(self.offschedule) - 1, 0, -1):
                    ...
                    if "КАФЕДР" in self.offschedule[i]:
                        self.offschedule[i] += " "
            ...
            for i in range(len(par)):
                ...
                for j in range(len(par[i])):
                    if "▼" in par[i][j] and "▲" in par[i][j]:
                        ...
                        self.weekdown[i][len(self.weekdown[i]) - 1].insert(0, par[i][j][
                            0])  # Добавление номера и времени пары для нижней недели
                    elif "▼" in par[i][j]:
    ...
    def url_ok(self):
        try:
            requests.head("https://guap.ru/rasp/")
            return True
        except:
            return False
